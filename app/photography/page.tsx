...
export default function Photography() {
  ...
  return (
    <div className="min-h-[90dvh] flex flex-col items-center gap-24 pb-32">
        ...
        {photographs.map((photo, i) => {
          return (
            <div title={photo} key={photo} className="cursor-pointer transition-all duration-100 hover:scale-[101%] photo-container w-full mb-1 md:mb-2 relative" onClick={() => { setZoomedImage(i) }}>
              <Image className='mx-auto' width={500} height={500} src={photo} alt="asdf" quality={50} loader={cloudfrontLoader} />
            </div>
          );
        })}
      </div>
      {zoomedImage !== undefined && (
        <div onKeyDown={(e) => {if (e.key == "Escape") setZoomedImage(undefined)}} onClick={() => setZoomedImage(undefined)} className="fixed bg-black/90 w-full h-[100dvh]">
          <div className='' >
            ...
            <div className='relative w-full h-full left-0 right-0 '>
            ...
            <footer className='relative height-[5dvh]'>
              <h1>footer</h1>
            </footer>
          </div>
        </div>
